...
- `USER_MARKET` (строка) - **ОБЯЗАТЕЛЬНО с февраля 2026**. Код вашего рынка/региона Spotify (например, `US`, `RU`, `GB`, `DE`). Spotify удалил определение страны из эндпоинта `/me`, поэтому это необходимо настроить вручную. Используется для:
  - Фильтрации недоступных треков с помощью `Filter.removeUnavailable()`
  - Поиска альбомов и треков с учетом рынка
  - Обеспечения доступности контента в вашем регионе

  !> Без `USER_MARKET` некоторые функции могут возвращать некорректные результаты или завершаться с ошибкой.

...
?> **ИЗМЕНЕНО** - Эта функция затронута изменениями Spotify API от февраля 2026. Теперь использует graceful fallback: топ треки Last.fm + рекомендации Spotify, когда прямой доступ к API недоступен. Результаты могут отличаться от предыдущего поведения.
...
?> **УСТАРЕВШИЕ ФИЛЬТРЫ** - Фильтры `popularity` и `followers` затронуты изменениями Spotify API от февраля 2026. Spotify больше не предоставляет точные данные для этих фильтров. Используйте `min_popularity` в `Source.getRecomTracks()` как альтернативу для фильтрации по популярности.
...
?> **УСТАРЕЛО** - Эта функция затронута изменениями Spotify API от февраля 2026. Эндпоинт категорий был удален.

**Альтернативы:**
- Используйте `Source.mineTracks()` для поиска плейлистов по ключевым словам
- Используйте `Source.getRecomTracks()` с `seed_genres` для рекомендаций по жанрам

...
### Версия 2.5.0 - Миграция Spotify API февраль 2026

**Критические изменения:**
- Spotify удалил определение страны из эндпоинта `/me`. **`USER_MARKET` теперь обязателен** в конфигурации. [Настройте его](/config) с кодом вашего рынка (например, `US`, `RU`, `GB`).
- Spotify ввел Development Mode с ограничениями: 1 Client ID на разработчика, требуется Premium подписка для расширения квоты. Подробности в [инструкции по установке](/install).
- `Source.getListCategory()` - **УДАЛЕНО** - Spotify прекратил поддержку эндпоинта категорий. Используйте `Source.mineTracks()` для поиска плейлистов по ключевым словам.
- `Source.getCategoryTracks()` - **УДАЛЕНО** - Spotify прекратил поддержку эндпоинта плейлистов категорий. Используйте `Source.mineTracks()` с ключевыми словами жанров.
- `Source.getArtistsTopTracks()` - Теперь использует graceful fallback (топ треки Last.fm + рекомендации Spotify). Результаты могут отличаться от предыдущего поведения.

**Устаревшие функции:**
- Фильтр `popularity` в `Source.getArtistsTracks()` - Spotify больше не предоставляет точные данные о популярности для фильтрации. Используйте `min_popularity` в рекомендациях `Source.getRecomTracks()` как альтернативу.
- Фильтр `followers` в `Source.getArtistsTracks()` - Больше не поддерживается Spotify API.

**Рекомендации:**
- Подайте заявку на **Extended Quota Mode** в Spotify Dashboard для полной функциональности (рекомендации, поиск и т.д.)
- Рассмотрите использование интеграции Last.fm как запасного варианта для рекомендаций исполнителей и топ треков
- Используйте `Source.getRecomTracks()` с `seed_genres` для рекомендаций по жанрам вместо просмотра категорий

**Требуется миграция:**
- Добавьте `USER_MARKET` в файл `config.gs` (обязательно для операций с учетом рынка)
- Пересмотрите скрипты, использующие `getListCategory()` или `getCategoryTracks()`, и мигрируйте на альтернативы
- При использовании фильтров `popularity` или `followers` - удалите или замените их

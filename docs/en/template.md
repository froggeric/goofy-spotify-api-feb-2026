...
Before using templates, it's recommended to familiarize yourself with the tutorial on creating the [first playlist](/first-playlist). It will significantly simplify working with templates and answer frequent questions. Additional templates can be found on the [GitHub forum](https://github.com/Chimildic/goofy/discussions) and [4PDA](https://4pda.to/forum/index.php?s=&showtopic=715234&view=findpost&p=101196000).
...
function updateUnknownSet() {
    ...
    // Insert playlist ids taken from link or URI
    // Example: https://chimildic.github.io/goofy/#/reference/desc?id=Identifier
    ...
    // Example of removing artists if you keep a ban playlist
    ...
    // Insert the created playlist id after first code run, remove comment
...
## New Releases in Chunks
```js
/**
 * When there are very many followed artists, there's a chance to reach the execution time limit or get a pause from Spotify for 24 hours.
 * You can split artists into chunks. For example, setting a trigger "every hour" with chunk size 100, 2400 artists will be checked per day.
 * The function accumulates releases in cache. If desired, you can mix them into other playlists.
 */
...
```js
/**
 * Popular tracks from albums that already have favorite tracks you know
 */
function updateDiscoveryAlbums() {
  ...
  Playlist.saveWithReplace({
    name: 'Album Discoveries',
    description: 'These tracks should appeal to you!',
...
```js
/**
 * Collect favorite tracks that haven't been listened to in a long time and were added more than a month ago
 * Recommended to use after accumulating at least some listening history
 */
...
```js
/**
 * Collect tracks from followed playlists that were added in the past week
 */
function updateFollowedTracks(){
    ...
    // Add Order and Selector to limit quantity

    Playlist.saveWithReplace({
        // id: 'yourId',
        name: 'Followed Updates',
...
```js
/**
 * Collect new releases from various editorial (curator) picks over the past week
 */
...
function updateCollectorPlaylist(){
    // After first playlist creation, add collection id and remove comments
    // const COLLECTOR_PLAYLIST_ID = '';
    let recentTracks = RecentTracks.get(1000);
    let newTracks = Source.getPlaylistTracks('Release Radar', 'yourId');

    // let currentTracks = Source.getPlaylistTracks('Collection: Release Radar', COLLECTOR_PLAYLIST_ID);
